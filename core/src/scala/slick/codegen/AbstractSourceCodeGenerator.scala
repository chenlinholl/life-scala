...
${result(rearranged)} // putting AutoInc last
    ...
    trait TableClassDef extends super.TableClassDef {
      ...
      def code = {
        ...
        s"""|class $name(tag: Tag) extends Table[$elementType](tag, ${args.mkString(", ")})$prns {
    ...
    trait TableValueDef extends super.TableValueDef {
      ...
      //      s"  $columnsMap\n" +
      //      "}"
      def code = s"\nval ${name.head.toLower + name.tail}s = new TableQuery(tag => new ${TableClass.name}(tag))"
    }
    ...
    class ColumnDef(model: m.Column) extends super.ColumnDef(model) {
      ...
      def code =
        s"""  val ${model.name}: Rep[$actualType] = column[$actualType]("${model.name}"${options.map(", " + _).mkString("")})"""
    }
