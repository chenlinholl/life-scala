...
abstract class AbstractGenerator[Code, TermName, TypeName](model: m.Model)
  ...
  /** Maps database table name to entity case class name
    *
    * @group Basic customization overrides */
  def entityName = (dbName: String) => dbName.toCamelCase + "Entity"
...
trait GeneratorHelpers[Code, TermName, TypeName] {
  def indent(code: String): String = {
    code.split("\n")
      .filterNot(_.contains("TableQuery"))
      .filterNot(_.contains("final case class"))
      .mkString("\n")
  }
  ...
  def indentCaseClass(code: String): String = {
    code.split("\n")
      .filterNot(_.contains("TableQuery"))
      .filter(_.contains("final case class"))
      .filterNot(_.isEmpty)
      .mkString("\n")
  }
